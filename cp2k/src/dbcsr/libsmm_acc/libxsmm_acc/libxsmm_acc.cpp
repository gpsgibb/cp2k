...
void libxsmm_acc_reconfigure()
{
  ...
  if (libxsmm_acc_private::reconfigure && !initialized) {
...
LIBXSMM_ACC_EXTERN
#endif
void LIBXSMM_ACC_FSYMBOL(LIBXSMM_ACC_CONCATENATE(LIBXSMM_ACC_CONCATENATE(__real_, CP2K_CONFIG_PREFIX), dbcsr_print_config))(LIBXSMM_ACC_CONFIG_SIGNATURE_DECL);
LIBXSMM_ACC_EXTERN void LIBXSMM_ACC_FSYMBOL(LIBXSMM_ACC_CONCATENATE(LIBXSMM_ACC_CONCATENATE(__wrap_, CP2K_CONFIG_PREFIX), dbcsr_print_config))(const int* unit_nr)
{
  libxsmm_acc_reconfigure();
  // call original dbcsr_print_config
  LIBXSMM_ACC_FSYMBOL(LIBXSMM_ACC_CONCATENATE(LIBXSMM_ACC_CONCATENATE(__real_, CP2K_CONFIG_PREFIX), dbcsr_print_config))(unit_nr);
}
