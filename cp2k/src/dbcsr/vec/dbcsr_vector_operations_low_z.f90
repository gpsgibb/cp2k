...
!$OMP          SHARED(matrix,fast_vec_col,fast_vec_row)
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       THEN
          CALL libxsmm_matmul(fast_vec_col%blk_map_z(prow)%ptr, data_d, &
                              fast_vec_row%blk_map_z(pcol)%ptr, transb='T')
       ELSE
#endif
...
! sum all the data onto the first processor col where the original vector is stored
    data_vec => dbcsr_get_data_p (work_col%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
...
! sum all the data within a processor column to obtain the replicated result
    data_vec => dbcsr_get_data_p (work_row%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
...
! Copy the local vector to the replicated on the first processor column (this is where vec_in lives)
    ...
    data_vec_rep => dbcsr_get_data_p (rep_col_vec%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
    data_vec => dbcsr_get_data_p (vec_in%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
...
! Hope this clarifies the idea
    ...
    data_vec_rep => dbcsr_get_data_p (rep_row_vec%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
       ...
       IF(ASSOCIATED(fast_vec_row%blk_map_z(pcol)%ptr) .AND.&
          ...
          IF(res_fast_vec_row%blk_map_z(rpcol)%assigned_thread .EQ. ithread .AND. row .NE. col) THEN
             ...
             IF (CP_VERSION4(1, 4, 1, 20).LE.CP_VERSION4( &
                ...
                LIBXSMM_VERSION_UPDATE, LIBXSMM_VERSION_PATCH)) &
             THEN
                CALL libxsmm_matmul(res_fast_vec_row%blk_map_z(rpcol)%ptr, &
                                    fast_vec_col%blk_map_z(prow)%ptr, &
                                    data_d, transa='T')
             ELSE
#endif
...
!$OMP END PARALLEL
    ...
    data_vec => dbcsr_get_data_p (result_row%data_area, select_data_type=CMPLX(0.0, 0.0, real_8))
